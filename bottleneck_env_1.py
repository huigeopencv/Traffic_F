...
class MergePOEnv(Env):
    ...
    * num_rl: maximum number of controllable vehicles in the network
    ...
    def observation_space(self):
        """See class definition."""
        return Box(-float('inf'), float('inf'), shape=(5 * self.num_rl, ), dtype=np.float32)
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        if self.env_params.evaluate:
            ...
            # rl_forward_progress(env, gain=0.1)

            # penalize small time headways
            cost2 = 0
            t_min = 1.5  # smallest acceptable time headway
            for rl_id in self.rl_veh:
                lead_id = self.k.vehicle.get_leader(rl_id)
                if lead_id not in ["", None] \
                        and self.k.vehicle.get_speed(rl_id) > 0 and self.k.vehicle.get_speed(rl_id) > self.k.vehicle.get_speed(lead_id):
                    t_headway = self.k.vehicle.get_headway(rl_id) / (self.k.vehicle.get_speed(rl_id)-self.k.vehicle.get_speed(lead_id))
                    cost2 += max((t_min - t_headway) / t_min, 0)
            # cost3 = rewards.penalize_near_standstill(self, thresh=4)
            rlids = self.k.vehicle.get_rl_ids()
            v_rl = [self.k.vehicle.get_speed(rl_id) for rl_id in rlids]
            L1 = len(v_rl)
            V_max = [i * 30 for i in [1] * L1]
            eps = np.finfo(np.float32).eps
            cost3 = max(0, np.linalg.norm([V_max[i] - v_rl[i] for i in range(L1)]) / (np.linalg.norm(V_max)+eps))
            # cost3 = 0
            eta1, eta2, eta3 = 5.00, -0.5, -0.6
            return max(eta1 * cost1 + eta2 * cost2 + eta3 * cost3, 0)
            # return eta1 * cost1 + eta2 * cost2 + eta3 * cost3
