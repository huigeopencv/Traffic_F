...
class BottleneckAccelEnv(BottleneckEnv):
    ...
    Actions
        ...
        动作空间由一个列表组成，其中前半部分是加速度，后半部分是我们绕过的车道变换方向
    ...
    Rewards
        ...
        奖励是网络中所有车辆的速度与某些期望速度之间的差值的两个标准。除此之外，我们还增加了车辆向前行驶的积极奖励，以及对频繁变道的车辆的处罚。
    ...
    def observation_space(self):
        ...
        # # print(num_edges,num_rl_veh,num_obs)
        #
        # return Box(-50000, 50000, shape=(num_obs, ), dtype=np.float32)
    ...
    def get_state(self):
        ...
        #
        # return np.concatenate((rl_obs, relative_obs))
    ...
    def _apply_rl_actions(self, actions):
        ...
        采用元组并应用车道变换或加速。如果应用了换道，在换道期间不要发出任何命令，
        并对换道期间的行为返回负奖励。如果没有应用变道，并且已经过了足够的时间，则发出正常的加速。
        """
        # num_rl = self.k.vehicle.num_rl_vehicles
        ...
        if actions:
            for rl_id, actions in actions.items():
                accel = actions[0]
    ...
    def additional_command(self):
        ...
        action space.
        这用于在系统中始终保持恒定数量的RL车辆，以符合固定尺寸的观察和行动空间。
        """
        super().additional_command()
...
class BottleneckDesiredVelocityEnv(BottleneckEnv):
    ...
    用于训练车辆有效通过瓶颈的环境，通过指定RL车辆应尝试在特定空间区域内行驶的速度。
    ...
    def get_state(self):
        ...
        系统的状态空间是通过将瓶颈分割成边缘，然后在每个边缘中分割来定义的。
        类变量 self.num_obs_segments指定每条边被切割成多少段。
        每条车道都定义了一个唯一的路段：我们称之为车道路段。
        例如，如果边1有四条车道和三条线段，那么我们总共有12条车道线段。
        我们将跟踪每个车道段中车辆的总统计数据。
    ...
    def reset(self):
        ...
        不同的流入集合被用来产生一个相对于流入率更为稳健的政策。
        通过创建与前一个网络类似的新网络来更新流入率，但新的流入对象的流入率在附加环境参数“inflow_range”内，
        该参数是由最小和最大允许流入率组成的列表。

        **WARNING**: The inflows assume there are vehicles of type
        "followerstopper" and "human" within the VehicleParams object.
        流入假设VehicleParams对象中存在“followerstopper”和“human”类型的车辆
        """
        add_params = self.env_params.additional_params
