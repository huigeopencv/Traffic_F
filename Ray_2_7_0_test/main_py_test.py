...
def setup_exps():
    ...
    config = (
        PPOConfig()
        .rollouts(num_rollout_workers=N_CPUS)
        .training(model={"fcnet_hiddens": [24, 24]}, train_batch_size=HORIZON * N_ROLLOUTS, gamma=0.99,lr=1e-5,lambda_=0.97, kl_target=0.02, num_sgd_iter=10)
        # 环境名字有问题
        .environment(env=gym_name, clip_actions=False)
