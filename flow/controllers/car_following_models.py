...
这些控制器可用于修改Flow中车辆的加速行为，以匹配可校准的各种主要跟车模型。
...
每个控制器包括函数“get_accel（self，env）->acc”，该函数使用当前世界状态和现有参数，使用控制模型返回车辆加速度。
"""
import math
import numpy as np
...
class SimCarFollowingController(BaseController):
    ...
    请注意，此处不提供通过BaseController实现噪声和故障保护的方法。然而，在初始化车辆参数时，可以通过sumo使用类似的方法。
