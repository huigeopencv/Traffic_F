...
class Env(gym.Env, metaclass=ABCMeta):
    ...
    提供用于与交通模拟的各个方面进行交互的界面。 使用此类，您可以启动模拟实例、提供网络以指定配置和控制器、执行模拟步骤以及将模拟重置为初始配置。

    Env is Serializable to allow for pickling and replaying of the policy.
    Env 是可序列化的，以允许对策略进行酸洗和重播。
    ...
    此类不能按原样使用：如果您选择将其与 rl 库（例如 RLlib）一起使用，则必须扩展它以实现动作应用程序方法和定义 MDP 的属性。 这可以通过在子类中重载以下函数来完成：
    ...
    * compute_reward
    ...
    def reset(self):
        ...
        positions.
        此方法在卷展栏之间执行。它将重置环境状态，并在其起始位置重新初始化车辆。
        ...
        如果InitialConfig中的“shuffle”设置为True，则车辆的初始位置将重新计算，车辆将被重新排列。
    ...
    def apply_rl_actions(self, rl_actions=None):
        ...
        如果在任何给定步骤中未提供任何操作，则rl代理默认执行SUMO指定的操作。
    ...
    def terminate(self):
        ...
        应在每次实验结束时进行。必须处于环境中，因为环境打开TraCI连接。
        """
        try:
