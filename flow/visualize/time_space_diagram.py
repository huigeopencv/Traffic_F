...
注意：此脚本假设所提供的网络在每条边缘上只有一条车道，或者在MergeNetwork的情况下，在主干道上只有一个车道。
...
from flow.networks import RingNetwork, FigureEightNetwork, MergeNetwork, I210SubNetwork, HighwayNetwork, BottleneckNetwork
...
def _get_abs_pos(df, params):
    ...
    if params['network'] == MergeNetwork:
        ...
        return df['x']
    elif params['network'] == BottleneckNetwork:
        return df['x']
...
def plot_tsd(ax, df, segs, args, lane=None, ghost_edges=None, ghost_bounds=None):
    ...
    ax : matplotlib.axes.Axes
        figure axes that will be plotted on将在其上绘制的图形轴
    df : pd.DataFrame
        data used for axes bounds and speed coloring用于轴边界和速度着色的数据
    segs : list of list of lists
        line segments to be plotted, where each segment is a list of two [x,y] pairs要绘制的线段，其中每条线段是两个[x，y]对的列表
    args : dict
        parsed arguments已解析的参数
    lane : int, optional
        lane number to be shown in plot title地块标题中显示的车道编号
    ghost_edges : list or set of str
        ghost edge names to be greyed out, default None重影边缘名称将变灰，默认为“无”
    ghost_bounds : tuple
        lower and upper bounds of domain, excluding ghost edges, default None域的下限和上限，不包括重影边，默认为“无”
    ...
    ax.set_ylabel('Position (m)', fontsize=32)
...
# 大多数用的字体大小是28,24(坐标轴数字的大小)
# 给图1的单独作以下修改,32,28
    cbar = plt.colorbar(lc, ax=ax, norm=norm)
    cbar.set_label('Speed (m/s)', fontsize=32)
    cbar.ax.tick_params(labelsize=28)
    # ax.set(gca,'LooseInset')
    fig.subplots_adjust(left=0.09,right=1.06,bottom=0.09,top=0.95)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--title', type=str, default='Time Space Diagram',
                        help='rate at which steps are plotted.')
    parser.add_argument('--max_speed', type=int, default=30,
