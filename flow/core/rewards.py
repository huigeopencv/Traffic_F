...
def desired_velocity(env, fail=False, edge_list=None):
    ...
    当环中的所有车辆都设置为该期望速度时，该功能测量车辆系统与用户指定的期望速度峰值的偏差。
    此外，为了确保奖励函数自然地惩罚由于冲突或其他故障而提前终止的推出，
    该函数被公式化为映射：math:\\mathcal｛S｝\\times\\mathcal{a｝\\rightarrow\\mathbb｛r｝_｛\\geq 0｝。
    这是通过从期望速度的峰值允许偏差中减去系统与期望速度的偏差来实现的。
    此外，由于车辆的速度在上方是无界的，因此奖励在下方以零为界，以确保非负。
...
def min_delay(env):
    ...
    用于鼓励最小化总延迟的奖励函数。
此功能测量车辆系统与以固定速度平稳行驶至目的地的所有车辆之间的偏差。
