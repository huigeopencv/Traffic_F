...
class Experiment:
    ...
    此类充当网络和环境的运行器。 为了在没有指定 RL 智能体在网络中的操作的方法的情况下使用它来运行网络和环境，请键入以下内容：
    ...
    如果您希望指定 RL 智能体在网络中的操作，可以按如下方式完成：
    ...
    最后，如果您想绘制和可视化您的结果，此类可以从 sumo 生成的发射文件生成csv文件。
    这些文件将包含网络中每辆车在每个时间步长的速度、位置、边缘等。
    ...
    为了确保模拟器构建发射文件，将 ``SimParams`` 中的 ``emission_path`` 属性设置为某个路径。
    ...
    实验完成后，查看“./data”目录。 将有两个文件，一个后缀为 .xml，另一个后缀为 .csv。
    后者应该可以从任何 csv 阅读器（例如 Excel）轻松解释，并且可以使用 numpy 和 pandas 等工具进行解析。
    ...
    custom_callables : dict < str, lambda >
        ...
        keyed by the str.
        字符串和 lambda 函数对应于我们想要从环境中提取的一些信息。 将在每一步调用 lambda以从 env 中提取信息，并将其存储在由str键入的字典中。
    env : flow.envs.Env
