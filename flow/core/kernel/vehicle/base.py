...
class KernelVehicle(object, metaclass=ABCMeta):
    ...
    methods for:
    该内核子类用于就所有与车辆相关的组件与模拟器进行交互。具体来说，此类包含以下方法：
    ...
    * Interacting with the simulator: This includes apply acceleration, lane
      ...
      methods.
      与模拟器交互：这包括对模拟器中的特定车辆应用加速、变道和路由命令。
      此外，存在从网络中添加或删除特定车辆的方法，并在每个模拟步骤后更新内部状态信息，以支持并可能加速所有状态获取方法。
    * Visually distinguishing vehicles by type: In the case when some vehicles
      ...
      按类型视觉区分车辆：在某些车辆由强化学习代理或其他控制器控制的情况下，这些方法可用于在渲染过程中通过RL/驱动、人类观察和人类未观察来视觉区分车辆。
      例如，traci模拟器将 RL车辆渲染为红色，将观察到的人类车辆渲染为青色，将未观察到的人类车辆渲染为白色。在没有RL/驱动代理的情况下，所有车辆都是白色的。
    * State acquisition: Finally, this methods contains several methods for
      ...
      be done by calling:
      状态获取：最后，该方法包含几种从特定车辆获取状态信息的方法。 例如，如果您想从环境中获取车辆的速度，可以通过调用：
    ...
    def remove(self, veh_id):
        ...
        this class.
        此方法从车辆内核和所有有效 ID 列表中删除车辆的所有痕迹，并减少此类中的车辆总数。
        ...
        此外，如果车辆仍在网络中，此方法会调用必要的模拟器特定命令将其删除。
    ...
    def apply_acceleration(self, veh_id, acc, smooth=True):
        ...
        在 SUMO 中，此函数应用应用平滑的 slowDown 方法。
    ...
    def apply_lane_change(self, veh_id, direction):
        ...
        此方法还可以防止车辆移动到不存在的车道，并为车道改变的 RL 车辆设置“last_lc”变量以匹配当前时间步长，以帮助维持这些车辆的车道改变持续时间。
    ...
    def get_last_lc(self, veh_id, error=-1001):
        ...
        注意：此值仅为 RL 车辆存储。 调用此方法的所有其他车辆将导致打印警告，并且它们的“last_lc”项将是错误值。
    ...
    def get_x_by_id(self, veh_id):
        ...
        注意：这些值只有在适当设置网络中的 specify_edge_starts 方法时才有意义； 否则，所有车辆返回值 0。
