...
class TraCIVehicle(KernelVehicle):
    ...
    def initialize(self, vehicles):
        ...
        它负责从VehicleParams对象中收集车辆类型信息，并将其放置在Vehicles内核中
    ...
    def update(self, reset):
        ...
        *修改所有车辆的状态以匹配当前时间步的状态。这包括sumo指定的状态和flow明确定义的状态，例如“num_arrived”
        ...
        *如果车辆退出网络，它们将从车辆类别中删除，新离开的车辆将被引入该类别。

        Parameters
        ----------
        reset : bool
            specifies whether the simulator was reset in the last simulation step
            指定模拟器是否在上一个模拟步骤中重置
        """
