...
class KernelSimulation(object):
    ...
    simulation.
    仿真内核负责生成仿真并将它们可用于与模拟交互的API传递给所有其他内核。
    ...
    child classes.这个类中的所有方法都是抽象的，必须被其他子类覆盖。
    """
    ...
    def start_simulation(self, network, sim_params):
        """Start a simulation instance. 启动模拟实例。
        ...
        network : any
            ...
            任何一个对象或变量，用于表示仿真期间使用的网络。例如，在SUMO仿真的情况下，这是（字符串）.sumo.cfg 文件的路径。
