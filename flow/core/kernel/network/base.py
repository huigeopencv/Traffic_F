...
class BaseKernelNetwork(object):
    ...
    该内核子类负责生成仿真交通网络所需的任何模拟特定组件。这可能包括网络创建配置文件，
    支持在模拟器（例如 sumo）中生成某些交通网络，或者可能就像从网络类（参见 flow/networks/base.py）传递网络特征并将它们传输到模拟器内核稍后。

    In addition to generating files for network initialization, the network
    kernel performs two auxiliary tasks:
    除了为网络初始化生成文件外，网络内核还执行两个辅助任务：
    ...
    * State acquisition: The network kernel contains several methods that can
      ...
      command:
      状态获取：网络内核包含几种方法，可用于获取有关正在模拟的网络属性的状态信息，
      例如 边上的车道数、边的长度、从起始位置开始的可用路线等...例如，如果您想确定车辆在网络中可以行驶的最大速度，这可以是 通过调用以下命令完成：
    ...
    * Methods for generating initial vehicle positions: Initial vehicle
      ...
      生成初始车辆位置的方法：初始车辆位置由抽象网络内核生成，如果 ``initial_config`` 中的间距设置为随机，
      则可能会被从 flow.networks.Network对象生成的网络覆盖。
      默认初始位置包括统一的起始位置（所有车辆的间距相等）或随机的起始位置（受某些 min_gap 限制）。
      有关如何增加车辆起始位置的更多详细信息，
      see:  # TODO: create tutorial
    """

    def __init__(self, master_kernel, sim_params):
        """Instantiate the base network kernel.实例化基础网络内核。
    ...
    def update(self, reset):
        ...
        由于网络通常是静态的，因此这很可能不包括正在执行的任何操作。 这主要是为了与其他子内核保持一致。
