...
class MultiAgentHighwayPOEnv(MultiEnv):
    ...
    * target_velocity: desired velocity for all vehicles in the network, in m/s
    ...
    #     rewards = {}
    #     max_speed = self.k.network.max_speed()
    #     rlids = self.k.vehicle.get_rl_ids()
    #     humanids = self.k.vehicle.get_human_ids()
    #     v_rl = [self.k.vehicle.get_speed(rl_id) for rl_id in rlids]
    #     v_human = [self.k.vehicle.get_speed(human_id) for human_id in humanids]
    #     L1 = len(v_rl)
    #     L2 = len(v_human)
    #     V_des = [i * max_speed for i in [1] * (L1 + L2)]
    #     V_min = [i * 5 for i in [1] * L1]
    #     v_total = v_rl + v_human
    #     # cost1 = max(0, (np.linalg.norm(V_des, ord=2) - np.linalg.norm(([V_des[i] - v_total[i] for i in range(L1 + L2)]),
    #     #                                                               ord=2)) / np.linalg.norm(V_des, ord=2))
    #     # cost2 = max(0, np.linalg.norm(([V_min[i] - v_rl[i] for i in range(L1)]), ord=2) / np.linalg.norm(V_min, ord=2))
    #     # eta1, eta2, eta3 = 2, -0.3, -0.3
    ...
    #         else:
    #             max_speed = self.k.network.max_speed()
    #         min_speed = self.k.network.max_speed()
    #         rlids = self.k.vehicle.get_rl_ids()
    #         humanids = self.k.vehicle.get_human_ids()
    #         v_rl = [self.k.vehicle.get_speed(rl_id) for rl_id in rlids]
    #         v_human = [self.k.vehicle.get_speed(human_id) for human_id in humanids]
    ...
    #         V_min = [i*5 for i in [1]*L1]
    #         v_total = v_rl+v_human
    #         cost1 = max(0, (np.linalg.norm(V_des, ord=2)-np.linalg.norm(([V_des[i]-v_total[i] for i in range(L1+L2)]), ord=2))/np.linalg.norm(V_des, ord=2))
    #         cost2 = max(0, np.linalg.norm(([V_min[i]-v_rl[i] for i in range(L1)]), ord=2)/np.linalg.norm(V_min, ord=2))
    #         # print(L1,L2,cost1,cost2)
    #         #     cost3 = 0
    #             t_min = 1.5  # smallest acceptable time headway 最小可接受时间间隔
    #         # 获取该车前方的车辆
    #             lead_id = self.k.vehicle.get_leader(rl_id)
    #             if lead_id not in ["", None] \
    #                     and self.k.vehicle.get_speed(rl_id) > 0:
    #                 # 如果前方车辆存在而且当前车辆的速度大于0
    #                 if self.k.vehicle.get_speed(rl_id) > self.k.vehicle.get_speed(lead_id):
    #                     t_headway = self.k.vehicle.get_headway(lead_id) / (
    #                                 self.k.vehicle.get_speed(rl_id) - self.k.vehicle.get_speed(lead_id))
    #                     cost3 = max((t_min - t_headway) / t_min, 0)
    #                 else:
    #                     cost3 = 0
    ...
    #     # for rl_id in self.k.vehicle.get_rl_ids():
    #     #     rewards[rl_id] = reward
    #     # print(len(rewards),min(rewards),max(rewards))
    #     # print(rewards)
    #     return rewards
    ...
    def compute_reward(self, rl_actions, **kwargs):
        ...
        for rl_id in self.k.vehicle.get_rl_ids():
            if kwargs['fail']:
                ...
                max_speed = self.k.network.max_speed()
            # cost = sum([(max_speed - self.k.vehicle.get_speed(j)) for j in self.k.vehicle.get_rl_ids()])
            # cost1 = cost/(max_speed*len(self.k.vehicle.get_rl_ids()))
                # # min_speed = self.k.network.max_speed()
                cost1 = max(0, (max_speed-self.k.vehicle.get_speed(rl_id))/max_speed)
                cost2 = max(0, (5-self.k.vehicle.get_speed(rl_id))/5)
                cost3 = 0
                t_min = 1.5  # smallest acceptable time headway 最小可接受时间间隔
                # 获取该车前方的车辆
                lead_id = self.k.vehicle.get_leader(rl_id)
                if lead_id not in ["", None] \
                        and self.k.vehicle.get_speed(rl_id) > 0 and self.k.vehicle.get_speed(
                    rl_id) > self.k.vehicle.get_speed(lead_id):
                    t_headway = self.k.vehicle.get_headway(rl_id) / (
                                self.k.vehicle.get_speed(rl_id) - self.k.vehicle.get_speed(lead_id))
                    cost3 = max((t_min - t_headway) / t_min, 0)
                eta1, eta2, eta3 = -5, -0.5, -0.6
                reward = eta1 * cost1 + eta2 * cost2 + eta3 * cost3
                # print(cost, len(self.k.vehicle.get_rl_ids()), cost1, reward)
        #
            rewards[rl_id] = reward
        # # print(rewards)
        return rewards
